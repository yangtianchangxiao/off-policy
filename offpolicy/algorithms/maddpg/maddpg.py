...
class MADDPG(Trainer):
    ...
    def cent_train_policy_on_batch(self, update_policy_id, batch):
        ...
        # critic update
        with torch.no_grad():
            # print(update_policy.target_critic(all_agent_cent_nobs, all_agent_cent_nact))
            # print(update_policy_id)
            next_step_Q = update_policy.target_critic(all_agent_cent_nobs, all_agent_cent_nact)[int(update_policy_id[-1])].reshape(-1, 1)
        if self.use_popart:
        ...
        predicted_Qs = update_policy.critic(all_agent_cent_obs, all_agent_cent_act_buffer)[int(update_policy_id[-1])].reshape(-1, 1)
        ...
        if update_actor:
            ...
            # print("len actor",len_actor_Qs)
            # print("len valid maks", len(valid_trans_mask))
            len_mask = len(valid_trans_mask)
            agents_num = int(len_actor_Qs/len_mask)
            for i in range(len_mask):
                for j in range(agents_num):
                # print("actor_Qs[i]",actor_Qs[i])
                # print("valid_trans_mask",valid_trans_mask)
                    actor_Qs[i] = actor_Qs[i*agents_num+j] * valid_trans_mask[i]
            actor_Qs = torch.cat(tuple(actor_Qs))
